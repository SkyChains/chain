...
func BootstrapNewNetwork(
	...
	if err := network.EnsureDefaultConfig(w, luxNodeExecPath, pluginDir); err != nil {
		return err
	}
...
func (n *Network) StartNodes(ctx context.Context, w io.Writer, nodesToStart ...*Node) error {
	...
	if _, err := fmt.Fprintf(w, "\nMetrics: https://grafana-experimental.lux-dev.network/d/kBQpRdWnk/lux-main-dashboard?&var-filter=network_uuid%%7C%%3D%%7C%s&var-filter=is_ephemeral_node%%7C%%3D%%7Cfalse&from=%d&to=now\n", n.UUID, startTime.UnixMilli()); err != nil {
		return err
	}
